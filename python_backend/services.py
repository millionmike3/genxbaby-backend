...
from merkle import build_merkle_root_for_properties
from blockchain import anchor_merkle_root_on_polygon
...
def link_property_to_bulk_tape(
    db: Session, data: BulkTapePropertyCreate
) -> BulkTapeProperty:
...
def anchor_bulk_tape(db: Session, req: MerkleAnchorRequest) -> MerkleAnchorOut:
    tape = db.query(BulkTape).filter(BulkTape.id == req.bulk_tape_id).first()
    if not tape:
        raise ValueError("Bulk tape not found")

    links = (
        db.query(BulkTapeProperty)
        .filter(BulkTapeProperty.bulk_tape_id == tape.id)
        .all()
    )
...
def get_admin_stats(db: Session):
    ...
    total_vault_balance = db.query(UserVault).with_entities(
        UserVault.balance
    ).all()
    total_vault_balance_sum = sum(b for (b,) in total_vault_balance)

    return {
        "total_users": total_users,
        "total_domains": total_domains,
        "total_properties": total_properties,
        "total_bulk_tapes": total_bulk_tapes,
        "total_vault_balance": float(total_vault_balance_sum),
    }
