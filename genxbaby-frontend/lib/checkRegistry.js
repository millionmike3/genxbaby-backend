...
async function main() {
  const action = process.argv[2]; // "deploy", "roles", "batch"
  ...
  if (action === "deploy") {
    ...
    if (action === "roles") {
      ...
      console.log("Usage:");
      console.log("  node scripts/check-registry.js deploy");
      console.log("  node scripts/check-registry.js roles");
      console.log("  node scripts/check-registry.js batch");
    }
  }
}
